...
import { AlertTriangle, Filter, Pencil, Plus, Squircle, Trash2 } from 'lucide-react';
import PageHeader from '../components/PageHeader';
import { Asset, Department, Employee } from '../models/types';
import { getAssets, createAsset, updateAsset, deleteAsset, getDepartments, getEmployees } from '../utils/dataService';
...
const AssetsPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!currentAsset.assetModel || !currentAsset.assetSN || !currentAsset.assetType || !currentAsset.assetStatus || !currentAsset.assetInventoryNumber) return;
    ...
    if (isEditing && currentAsset.id) {
      ...
      createAsset({
        ...
        assetType: currentAsset.assetType as 'computer' | 'smartphone' | 'dockstation' | 'laptop' | 'monitor',
        assetStatus: currentAsset.assetStatus as 'inOperation' | 'inRepaire' | 'inStock' | 'writeOff',
        ...
        purchasePrice: currentAsset.purchasePrice ? Number(currentAsset.purchasePrice) : undefined,
  ...
  const filterAssetsByPermission = (assets: Asset[]): Asset[] => {
    ...
    if (currentUser.role === 'MANAGER') {
      ...
      return assets.filter(asset => 
        ...
        (asset.employee && getEmployees().some(emp => 
    ...
    if (currentUser.role === 'USER') {
      ...
      return assets.filter(asset => 
        asset.employee?.includes(`${currentUser.firstName} ${currentUser.lastName}`)
      );
    }
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('assetInventoryNumber')}>
                  <div className="flex items-center">
                    Inventory #
                    {sortField === 'assetInventoryNumber' && (
                      sortDirection === 'asc' ? <Squircle className="h-4 w-4 ml-1" /> : <Squircle className="h-4 w-4 ml-1" />
                    )}
                  </div>
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('assetModel')}>
                  <div className="flex items-center">
                    Model
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('assetSN')}>
                  <div className="flex items-center">
                    Serial Number
                    {sortField === 'assetSN' && (
                      sortDirection === 'asc' ? <Squircle className="h-4 w-4 ml-1" /> : <Squircle className="h-4 w-4 ml-1" />
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('assetType')}>
                  <div className="flex items-center">
                    Type
                    {sortField === 'assetType' && (
                      sortDirection === 'asc' ? <Squircle className="h-4 w-4 ml-1" /> : <Squircle className="h-4 w-4 ml-1" />
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('assetStatus')}>
                  <div className="flex items-center">
                    Status
                ...
                sortedAndFilteredAssets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(asset.assetStatus)}`}>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label htmlFor="assetInventoryNumber" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                  <label htmlFor="assetModel" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                  <label htmlFor="assetSN" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={(() => {
                      ...
                      const matchingEmployee = employees.find(
                        emp => `${emp.firstName} ${emp.lastName}` === currentAsset.employee
                      );
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {currentAsset.departmentId && `Department: ${departments.find(d => d.id === currentAsset.departmentId)?.name || 'Unknown'}`}
                  </p>
                </div>
