...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const canViewAsset = (assetId: string): boolean => {
    ...
    if (currentUser.role === 'MANAGER') {
      ...
      const departmentEmployees = employees.filter(e => e.departmentId === currentUser.departmentId);
    ...
    if (currentUser.role === 'USER') {
      return asset.employee?.includes(`${currentUser.firstName} ${currentUser.lastName}`) || false;
    }
  ...
  const canViewEmployee = (employeeId: string): boolean => {
    ...
    return `${currentUser.firstName} ${currentUser.lastName}` === `${employee.firstName} ${employee.lastName}`;
  };
